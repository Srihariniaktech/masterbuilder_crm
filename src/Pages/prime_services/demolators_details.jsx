...
const DEMOLATORS_API_URL = "https://masterbuilder-backend.onrender.com/api/manpower/details";
const UPLOAD_SINGLE = "https://masterbuilder-backend.onrender.com/api/upload/single";

export default function DemolatorDetails() {
    const { categoryId } = useParams();
    const navigate = useNavigate();
    const [category, setCategory] = useState(null);
    const [demolators, setDemolators] = useState([]);
    ...
    const fetchDemolators = async () => {
        try {
            const res = await axios.get(DEMOLATORS_API_URL);
            let data = Array.isArray(res.data) ? res.data : (res.data.data || []);
            const filtered = data.filter(w => String(w.primecategoriesid) === String(categoryId));
            setDemolators(filtered);
        } catch (err) { console.error("Demolators fetch error", err); }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (editId) {
                await axios.put(`${DEMOLATORS_API_URL}/${editId}`, payload);
                alert("✅ Record updated!");
            } else {
                await axios.post(DEMOLATORS_API_URL, payload);
    ...
    const handleDelete = async (id) => {
        ...
        try {
            await axios.delete(`${DEMOLATORS_API_URL}/${id}`);
    ...
    return (
        <div style={styles.container}>
            ...
            <h2 style={styles.mainTitle}>{category ? category.name.toUpperCase() : "LOADING..."} PROFILES</h2>
            ...
            <div style={styles.formBox}>
                ...
                <form onSubmit={handleSubmit} style={styles.form}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px" }}>
                        <input type="text" placeholder="Specialty 1" value={formData.services?.s1 || formData.services?.repair || ""} onChange={e => setFormData({ ...formData, services: { ...formData.services, s1: e.target.value } })} style={styles.inputStyle} />
                        <input type="text" placeholder="Specialty 2" value={formData.services?.s2 || formData.services?.install || ""} onChange={e => setFormData({ ...formData, services: { ...formData.services, s2: e.target.value } })} style={styles.inputStyle} />
                    </div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px" }}>
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "20px" }}>
                            ...
                            <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                                ...
                                <div style={{ display: "flex" }}>
                                    <input type="file" id="demoBannerFile" accept="image/*" style={{ display: "none" }} onChange={(e) => handleUpload(e, "bannerurl")} />
                                    <label htmlFor="demoBannerFile" style={styles.uploadBtnSvc}>📁 UPLOAD BANNER</label>
                                </div>
                            ...
                            <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                                ...
                                <div style={{ display: "flex" }}>
                                    <input type="file" id="demoLogoFile" accept="image/*" style={{ display: "none" }} onChange={(e) => handleUpload(e, "profileUrl")} />
                                    <label htmlFor="demoLogoFile" style={styles.uploadBtnSvc}>📁 UPLOAD LOGO</label>
                                </div>
                            ...
                            <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                                ...
                                <div style={{ display: "flex" }}>
                                    <input type="file" id="demoGalleryFile" accept="image/*" multiple style={{ display: "none" }} onChange={(e) => handleUpload(e, "gallery")} />
                                    <label htmlFor="demoGalleryFile" style={styles.uploadBtnSvc}>📁 ADD PHOTOS</label>
                                </div>
            ...
            <div style={styles.cardList}>
                {demolators.map(unit => {
                    const specialties = [unit.services?.s1 || unit.services?.repair, unit.services?.s2 || unit.services?.install, unit.services?.s3, unit.services?.s4].filter(Boolean);
...
const styles = {
    ...
    /* Builder Cards identical matched styles */
    cardList: {
        display: "grid",
